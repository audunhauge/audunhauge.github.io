...
const box = new Sprite({ div: getdiv(), x: 0, y: 0, h: 600, w: 600 });
...
function setup() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    let r = new Movable({ div: getdiv(), x, y, w: 10, h: 10 }, 3, 4);
    r.div.className = "child";
  ...
  setInterval(() => {
    for (let item of itemList) {
      ...
      if (item instanceof Movable) {
        ...
        item.edge(box);
        if (item instanceof Movable && item !== m) {
